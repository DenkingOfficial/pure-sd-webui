...
def create_seed_inputs(target_interface):
    with FormRow(elem_id=f"{target_interface}_seed_row", variant="compact"):
        ...
        random_seed = ToolButton(random_symbol, elem_id=f"{target_interface}_random_seed")
        reuse_seed = ToolButton(reuse_symbol, elem_id=f"{target_interface}_reuse_seed")
        seed_checkbox = gr.Checkbox(label='Extra', elem_id=f"{target_interface}_subseed_show", value=False, visible=True)
...
def create_ui(startup_timer = None):
                    ...
                    if category == "sampler":
                        ...
                        with FormGroup(visible=False, elem_id="txt2img_second_pass") as hr_options:
                            ...
                            with FormRow(elem_id="txt2img_refiner_row3", variant="compact"):
                                refiner_prompt = gr.Textbox(value='', label='Refiner prompt')
                            with FormRow(elem_id="txt2img_refiner_row4", variant="compact"):
                                refiner_negative = gr.Textbox(value='', label='Refiner negative prompt')
    ...
    interfaces = [
        ...
        (extras_interface, "Extras", "process"),
    ...
    with gr.Blocks(theme=modules.shared.gradio_theme, analytics_enabled=False, title="Stable Diffusion WebUI", allowed_paths=[cmd_opts.data_dir]) as demo:
